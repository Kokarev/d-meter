...
enum QuantityRange {
  // Ranges are contiguous: [min, max) for all except last which is [min, max].
  // Adjacent ranges share a boundary (r1to100.max == r101to1000.min == 100)
  // so values like 100.07 never fall into a gap.
  r1to100    (min: 1,      max: 100),     // танкери до 100 м³ / бензовози
  r101to1000 (min: 100,    max: 1000),    // нафтобази
  r1001to10000(min: 1000,  max: 10000),   // танкерний флот
  r10kto100k (min: 10000,  max: 100000);  // нафтозаводи, панамакси
  ...
  String get label {
    if (max <= 100)    return '1–100';
    if (max <= 1000)   return '101–1 000';
    if (max <= 10000)  return '1 001–10 000';
    return '10 001–100 000';
  }

  /// Half-open interval [min, max). Last range is closed [min, max].
  /// This ensures adjacent ranges share their boundary without overlap or gap.
  bool contains(double value) {
    if (this == QuantityRange.r10kto100k) return value >= min && value <= max;
...
class VisualizationState extends ChangeNotifier {
  ...
  QuantityRange _qRange = QuantityRange.r101to1000;
  double _qValue = 120.0;
  ...
  ThermalState _computeState(double tempC) {
    final refState = DensityCurveService.computeThermalState(
      p15KgM3: _p15KgM3,
      family: _fuel.family,
      tempC: tempC,
      massT: 1.0,
...
final densityAtT = refState.densityKgL;
